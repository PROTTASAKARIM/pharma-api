...
const updateInventoryOutOnDamageIn = async (req, res, next) => {
  ...
  //     createdAt: { $gte: startOfDay(new Date()), $lte: endOfDay(new Date()) },
  // });
  const products = await req.body.product
          ...
          if (success.priceTable.length > 0) {
            ...
            if (checked?.length > 0) {
              ...
              inventory = {
                ...
                createdAt:
                  success.createdAt !== undefined
                    ? new Date(Date.parse(success.createdAt))
                    : new Date(Date.now()),
                ...
                priceTable: [
                  ...
                  {
                    id: checked[0].get("id"),
                    currentQty: checked[0].get("currentQty")
                      ? Number(checked[0].get("currentQty")) - Number(qty)
                      : 0,
                    openingQty: checked[0].get("openingQty")
                      ? checked[0].get("openingQty") - Number(qty)
                      : 0 - Number(qty),
                    totalQty: checked[0].get("totalQty")
                      ? checked[0].get("totalQty") - Number(qty)
                      : 0 - Number(qty),
                    soldQty: checked[0].get("soldQty")
                      ? checked[0].get("soldQty")
                      : 0,
                    damageQty: checked[0].get("damageQty")
                      ? Number(checked[0].get("damageQty")) + Number(qty)
                      : 0 + Number(qty),
                    rtvQty: checked[0].get("rtvQty")
                      ? checked[0].get("rtvQty")
                      : 0,
                  },
                ],
              };
                ...
                priceTable: [
                  ...rest,
                  {
                    ...
                    soldQty: checked[0].get("soldQty")
                      ...
                      : 0,
                    damageQty: checked[0].get("damageQty")
                      ? Number(checked[0].get("damageQty"))
                      : 0,
                    rtvQty: checked[0].get("rtvQty")
                      ? checked[0].get("rtvQty") + Number(qty)
                      : 0 + Number(qty),
                  },
                ],
              };
                ...
                priceTable: [
                  ...rest,
                  {
                    ...
                    currentQty: checked[0].get("currentQty")
                      ...
                      : 0 + Number(qty),
                    openingQty: checked[0].get("openingQty")
                      ? checked[0].get("openingQty")
                      : 0,
                    totalQty: checked[0].get("totalQty")
                      ? checked[0].get("totalQty")
                      : 0,
                    soldQty: checked[0].get("soldQty")
                      ? checked[0].get("soldQty") - Number(qty)
                      : 0 - Number(qty),
                    damageQty: checked[0].get("damageQty")
                      ? Number(checked[0].get("damageQty"))
                      : 0,
